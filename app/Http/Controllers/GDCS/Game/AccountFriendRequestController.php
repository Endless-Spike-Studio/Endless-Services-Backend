...
class AccountFriendRequestController extends Controller
{
    ...
    public function send(AccountFriendRequestSendRequest $request): int
    {
        ...
        if (!$target) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_friend_request_send_failed_target_not_found'), Response::GAME_ACCOUNT_FRIEND_REQUEST_CREATE_FAILED_NOT_FOUND->value);
        }
        ...
        if ($targetHasBlockedOperator) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_friend_request_send_failed_blocked_by_target'), Response::GAME_ACCOUNT_FRIEND_REQUEST_CREATE_FAILED_BLOCKED_BY_TARGET->value);
        }

        if ($target->setting->friend_request_state === AccountSettingFriendRequestState::NONE) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_friend_request_send_failed_blocked_by_target_setting'), Response::GAME_ACCOUNT_FRIEND_REQUEST_CREATE_FAILED_TARGET_DISABLED->value);
        }
    ...
    public function index(AccountFriendRequestFetchRequest $request): int|string
    {
        ...
        $count = $query->count();
        if ($count <= 0) {
            throw new GeometryDashChineseServerException(__($getSent ? 'gdcn.game.error.account_friend_request_send_failed_empty_sent' : 'gdcn.game.error.account_friend_request_send_failed_empty'), Response::GAME_ACCOUNT_FRIEND_REQUEST_INDEX_FAILED_EMPTY->value);
        }
    ...
    public function accept(AccountFriendRequestAcceptRequest $request): int
    {
        ...
        if (!$friendRequest) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_friend_request_accept_failed_not_found'), Response::GAME_ACCOUNT_FRIEND_REQUEST_ACCEPT_FAILED_NOT_FOUND->value);
        }

        if ($friendRequest->account_id !== $data['targetAccountID']) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_friend_request_accept_failed_target_account_not_match'), Response::GAME_ACCOUNT_FRIEND_REQUEST_ACCEPT_FAILED_TARGET_ACCOUNT_NOT_MATCH->value);
        }

        if ($friendRequest->target_account->isNot($request->account)) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_friend_request_accept_failed_not_receiver'), Response::GAME_ACCOUNT_FRIEND_REQUEST_ACCEPT_FAILED_NOT_RECEIVER->value);
        }
    ...
    public function delete(AccountFriendRequestDeleteRequest $request): int
    {
        ...
        if (!$query->exists()) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_friend_request_delete_failed_not_found'), Response::GAME_ACCOUNT_FRIEND_REQUEST_DELETE_FAILED_NOT_FOUND->value);
        }
