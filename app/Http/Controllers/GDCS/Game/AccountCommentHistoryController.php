...
class AccountCommentHistoryController extends Controller
{
    ...
    public function index(AccountCommentHistoryFetchRequest $request): int|string
    {
        ...
        if ($count <= 0) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_comment_history_index_failed_empty'), response: Response::GAME_ACCOUNT_COMMENT_HISTORY_INDEX_FAILED_EMPTY->value);
        }
        ...
        switch ($data['mode']) {
            ...
            case CommentMode::MOST_LIKED->value:
                ...
                break;
            default:
                throw new GeometryDashChineseServerException(__('gdcn.game.error.account_comment_history_index_failed_invalid_mode'), response: Response::GAME_ACCOUNT_COMMENT_HISTORY_INDEX_FAILED_INVALID_MODE->value);
        }
