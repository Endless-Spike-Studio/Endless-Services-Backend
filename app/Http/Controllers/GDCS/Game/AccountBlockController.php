...
class AccountBlockController extends Controller
{
    ...
    public function block(AccountBlockRequest $request): int
    {
        ...
        if (AccountBlock::where($context)->exists()) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_block_failed_already_exists'), Response::GAME_ACCOUNT_BLOCK_FAILED_ALREADY_EXISTS->value);
        }
    ...
    public function unblock(AccountUnblockRequest $request): int
    {
        ...
        if (!$query->exists()) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_unblock_failed_not_found'), Response::GAME_ACCOUNT_UNBLOCK_FAILED_NOT_FOUND->value);
        }
