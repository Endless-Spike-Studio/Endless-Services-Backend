...
class AccountMessageController extends Controller
{
    ...
    public function send(AccountMessageSendRequest $request): int
    {
        ...
        if (!$target) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_message_send_failed_target_not_found'), game_response: Response::GAME_ACCOUNT_MESSAGE_CREATE_FAILED_TARGET_NOT_FOUND->value);
        }
        ...
        if ($targetHasBlockedOperator) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_message_send_failed_blocked_by_target'), game_response: Response::GAME_ACCOUNT_MESSAGE_CREATE_FAILED_BLOCKED_BY_TARGET->value);
        }

        if ($target->setting->message_state === AccountSettingMessageState::NONE) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_message_send_failed_blocked_by_target_setting'), game_response: Response::GAME_ACCOUNT_MESSAGE_CREATE_FAILED_TARGET_DISABLED->value);
        }
    ...
    public function index(AccountMessageFetchRequest $request): string
    {
        ...
        $count = $query->count();
        if ($count <= 0) {
            throw new GeometryDashChineseServerException(__($getSent ? 'gdcn.game.error.account_message_index_failed_empty_sent' : 'gdcn.game.error.account_message_index_failed_empty'), game_response: Response::GAME_ACCOUNT_MESSAGE_INDEX_FAILED_EMPTY->value);
        }
    ...
    public function fetch(AccountMessageDownloadRequest $request): string
    {
        ...
        if (!$message) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_message_fetch_failed_not_found'), game_response: Response::GAME_ACCOUNT_MESSAGE_FETCH_FAILED_NOT_FOUND->value);
        }
        ...
        if ($owner->isNot($request->account)) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_message_fetch_failed_not_owner'), game_response: Response::GAME_ACCOUNT_MESSAGE_FETCH_FAILED_NOT_OWNER->value);
        }
    ...
    public function delete(AccountMessageDeleteRequest $request): int
    {
        ...
        if (!$query->exists()) {
            throw new GeometryDashChineseServerException(__('gdcn.game.error.account_message_delete_failed_not_found'), game_response: Response::GAME_ACCOUNT_MESSAGE_DELETE_FAILED_NOT_FOUND->value);
        }
